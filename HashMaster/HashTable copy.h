...
public:
    ...
    // Key enumeration utility
    std::string get_key_value(const DataIndexEntry* entry) const;
    
    // DataIndexEntry iterator (STL-style)
    class DataIterator {
    private:
        HashTable* _table;
        int _current_index;
    ...
    public:
        DataIterator(HashTable* table, int start_index = 0) 
            : _table(table), _current_index(start_index) {
            _max_entries = table ? table->_data_count : 0;
            if (_current_index == 0) {
                advance_to_next_occupied();  // 첫 번째 occupied entry로 이동
            }
        }
        ...
        DataIndexEntry* operator*();
        DataIterator& operator++();
        
        // Additional utility methods
        std::string get_current_key_value() const;
        bool is_valid() const { return _current_index < _max_entries; }
    };
    
    DataIterator begin() { return DataIterator(this, 0); }
    DataIterator end() { return DataIterator(this, _data_count); }
};
